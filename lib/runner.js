...
function run(filePath, key) {
    ...
    return Promise.all(pagesArr).then((pages) => {
        ...
        let pageSummary = pages.map(page => {
            ...
            if (page.data.lighthouse) {
                pageResult.lighthouse = page.data.summary.replace('results.php', 'lighthouse.php');
                pageResult.scores = summary.getScores(page.data.lighthouse);
            }

            return pageResult;
        });

        result.pages = pageSummary;
